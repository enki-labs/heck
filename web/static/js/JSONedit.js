...
app.directive('json', function($compile, $timeout) {
  return {
    ...
    link: function(scope, element, attributes) {
        ...
        if (typeof scope.save !== "undefined") {
            var newElement = angular.element('<button type="button" class="btn btn-primary btn-xs" ng-click="save(scope, true)"><i class="fa fa-save icon-white"> save</i></button> <button type="button" class="btn btn-primary btn-xs" ng-click="save(scope, false)"><i class="fa fa-undo icon-white"> undo</i></button><br/>');
        ...
        scope.toggleCollapse = function() {
            if (scope.collapsed) {
                ...
                scope.chevron = "fa fa-caret-square-o-down icon-primary";
            } else {
                scope.collapsed = true;
                scope.chevron = "fa fa-caret-square-o-right icon-primary";
            }
        };
        ...
        '<div ng-switch on="showAddKey" class="block" >'
            ...
            + '<span ng-switch-default>'
                // plus button
                + '<button class="addObjectItemBtn" ng-click="$parent.showAddKey = true"><i class="fa fa-plus-square icon-primary"></i></button>'
            + '</span>'
        + '</div>';
        ...
        if (scope.type == "object"){
            ...
            + ' ng-init="chevron = \'fa fa-caret-square-o-down icon-primary\'"></i>'
            ...
            + '<div class="jsonContents" ng-hide="collapsed">'
                ...
                + '<span class="block" ng-hide="key.indexOf(\'_\') == 0" ng-repeat="(key, val) in child">'
                    ...
                    + '<span class="jsonObjectKey">'
                        ...
                        // delete button
                        + '<i class="deleteKeyBtn fa fa-minus-square icon-primary" ng-click="deleteKey(child, key)"></i>'
                    + '</span>'
                    // object value
            ...
            var template = '<i ng-click="toggleCollapse()" ng-class="chevron" ng-init="chevron = \'fa fa-caret-square-o-down icon-primary\'"></i>'
            ...
            + '<div class="jsonContents" ng-hide="collapsed">'
                + '<ol class="arrayOl" ui-multi-sortable ng-model="child">'
                    ...
                    + '<li class="arrayItem" ng-repeat="val in child" ng-init="key=$index">' //key needed in moveKey()
                        // delete button
                        + '<i class="deleteKeyBtn fa fa-minus-square icon-primary" ng-click="deleteKey(child, $index)"></i>'
                        + '<i class="moveArrayItemBtn fa fa-align-justify icon-primary"></i>'
