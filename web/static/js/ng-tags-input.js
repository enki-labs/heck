            ...
            this.registerAutocomplete = function() {
                ...
                return {
                    tryAddTag: function(tag) {
                        if (tag.indexOf(":") == -1) { $scope.newTag = tag + ":"; }
                        else { $scope.newTag = tag; }
                        return $scope.tryAdd();
                    },
                    focusInput: function() {
                        input[0].focus();
...
tagsInput.directive('autoComplete', ["$document","$timeout","$sce","tagsInputConfig", function($document, $timeout, $sce, tagsInputConfig) {
    function SuggestionList(loadFn, loadValueFn, options) {
    ...
    return {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            suggestionList = new SuggestionList(scope.source, scope.source_value, scope.options);
